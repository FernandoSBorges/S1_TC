...
class CreateCellPositions():
    ...
    @staticmethod
    def ring_population(num_cells, regions, relative_density, center_point=500, y_interval=[890, 1090], seed=None, save_fig=None, map_color='viridis'):
        """
        ...
        if save_fig is not None: CreateCellPositions.plot_ring_population(center_point, regions, y_interval, cell_positions_3D, save_fig, map_color, xy_view = 'XZ_distance')

        return cell_positions_3D

    # Extract x, y, and z coordinates for the updated case
    @staticmethod
    def plot_ring_population(center_point, regions, y_interval, cell_positions_3D, save_fig=None, map_color='heat', distance_plane='xz', xy_view = 'trueXY'):
        ...
        ax1.scatter(x_updated_params, y_updated_params, z_updated_params, c=y_updated_params, cmap=map_color, marker='o',
        ...
        ax2.scatter(x_updated_params, z_updated_params, c=y_updated_params, cmap=map_color, marker='o',
                    label='Cell Positions')
        ax2.set_xlabel('X')
        ax2.set_ylabel('Z')
        ...
        circle1 = plt.Circle((center_point, center_point), max_radius, edgecolor='r', facecolor='none', linestyle='dotted') # Create a circle with center (center_point, center_point) and radius max_radius
        circle2 = plt.Circle((center_point, center_point), 280, edgecolor='b', facecolor='none', linestyle='dotted')        # Create a circle with center (center_point, center_point) and radius max_radius
        ...
        if xy_view == 'trueXY':
            ...
            line11 = plt.Line2D([center_point-(max_radius), center_point-(max_radius)], y_interval, linestyle='dotted', color='r')
            line12 = plt.Line2D([center_point+(max_radius), center_point+(max_radius)], y_interval, linestyle='dotted', color='r')
            line21 = plt.Line2D([center_point-280,          center_point-280],          y_interval, linestyle='dotted', color='b')
            line22 = plt.Line2D([center_point+280,          center_point+280],          y_interval, linestyle='dotted', color='b')
            ax3.add_line(line11)
            ax3.add_line(line12)
            ax3.add_line(line21)
            ax3.add_line(line22)
            ...
            line12 = plt.Line2D([(max_radius), (max_radius)], y_interval, linestyle='dotted', color='r')
            line22 = plt.Line2D([280,          280],          y_interval, linestyle='dotted', color='b')
            ax3.add_line(line12)
            ax3.add_line(line22)
            ax3.scatter(distance_from_center, y_updated_params, c=y_updated_params, cmap=map_color, marker='o',
                        label='Cell Positions')
            ax3.invert_yaxis()
    ...
    def plot_histogram(ax, distance_from_center, max_radius, distance_plane, norm_hist=True):
        ...
        ax.bar(bin_edges[:-1], hist_data, width=bin_width, color='b', edgecolor='black', alpha=0.7)
        if distance_plane == 'xz':  
...
if __name__ == '__main__':
    ...
    ring_pop_dicts={
                'CT_pop':{
                    'num_cells':            num_cells,
                    'regions':              [[0, 50], [50, 100], [100, 150], [150, 250], [250, 350]],
    ...
    for pop_name_ind, pop_name in enumerate(ring_pop_dicts.keys()):
        ...
        ring_pop_dicts[pop_name].update({'cell_positions_3D':CreateCellPositions.ring_population(num_cells, regions, relative_density, center_point, y_interval, seed, save_fig='CreateCellPositions_'+str(pop_name_ind)+'_'+pop_name+'_plot_ring_network_seed_'+str(seed)+'.png')})
